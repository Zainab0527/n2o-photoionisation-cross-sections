...
our_data = np.loadtxt('cross-sections-3sigma.txt', delimiter=':')
E_our = our_data[:, 0]
sigma_our = our_data[:, 1]

exp_data = np.loadtxt('lit_Xsec_experiment_3sigma.csv', delimiter=',')
E_exp = exp_data[:, 0]
sigma_exp = exp_data[:, 1]

rchf_data = np.loadtxt('lit_Xsec_RCHF_3sigma.csv', delimiter=',')
E_rchf = rchf_data[:, 0]
sigma_rchf = rchf_data[:, 1]

fchf_data = np.loadtxt('lit_Xsec_FCHF_3sigma.csv', delimiter=',')
E_fchf = fchf_data[:, 0]
...
plt.title(r'$3\sigma$ Photoionisation Cross Section of N$_2$O')

plt.axvline(x=408.5, color='gray', linestyle='--',
            linewidth=1, alpha=0.5, label=r'$3\sigma$ threshold')

plt.tight_layout()
plt.savefig('3sigma_cross_section_plots.png', dpi=300)
plt.show()